...
export default function KeyValueInstructionsShow({ property, record }: Props) {
  ...
  return (
    <div style={{ padding: '4px 0' }}>
      ...
      {Object.entries(parsed).map(([lang, steps]) => {
        if (!Array.isArray(steps)) return null;
        return (
          <div key={lang} style={{ marginBottom: '8px' }}>
            <div style={{ fontSize: '11px', color: '#adb5bd', marginBottom: '2px' }}>{lang.toUpperCase()}</div>
            <ol style={{ margin: '0 0 0 20px', padding: '0', fontSize: '13px' }}>
              {steps.map((step, idx) => (
                <li key={idx} style={{ marginBottom: '4px' }}>{String(step)}</li>
              ))}
            </ol>
          </div>
        );
