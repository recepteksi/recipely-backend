...
export class AuthController {
  ...
  handleRegister = async (req: Request, res: Response): Promise<void> => {
    ...
    const { email, expiresInSeconds, expiresAt, code } = result.value;
    res.status(200).json({
      message: this.ts.t('auth.register_code_sent', locale),
      email,
      expiresInSeconds,
      expiresAt: expiresAt.toISOString(),
      ...(this.exposeDevCode && code !== undefined ? { devCode: code } : {}),
    });
  };
  ...
  handleResendRegistrationCode = async (req: Request, res: Response): Promise<void> => {
    ...
    // Surface real failures (infra errors -> 500) instead of masking them as a
    // success. The generic-200 enumeration guard below still applies to the
    // found/not-found *success* split.
    if (!result.ok) {
      const { status, body: err } = failureToHttp(
        result.failure,
        (key) => this.ts.t(key, locale),
    ...
    const extra = value.found
      ? {
          expiresInSeconds: value.expiresInSeconds,
          expiresAt: value.expiresAt.toISOString(),
          ...(this.exposeDevCode && value.code !== undefined ? { devCode: value.code } : {}),
        }
      : {};
    res.status(200).json({
